...
* **quick jumps**: Used to help learn to time quick jumps (pressing A to shorten Mario's crouching animation before jumping on an enemy) in battle. Displays log messages with frame data.
* **hammer cancels**: Used to help learn to time hammer cancels (pressing B again shortly after hammering to gain control of Mario again faster). Displays log messages with frame data.
...
* **spin**: Used to help practice spin movement in the overworld. `timing bar` enables a visual display with a green bar and yellow marker that indicates the optimal frame to jump out of your spin. Below the bar is information your jump timing and start delay (explained below). `detailed stats` pins a separate customizable display with more detailed spin information. `jump timing` is the number of frames early/late you were when jumping out of your last spin. `jump length` is the number of frames A was held when jumping out of your last spin (1 is optimal). `start delay` is the number of wasted frames between when you were first able to spin again following your previous spin and when you actually began spinning. `z buffer` is the number of frames you pressed Z early when attempting to buffer your next spin.
* **ice staircase skip**: JP-exclusive. Used to help line up in the proper position to perform the Ice Staircase Skip trick. `position` can be either **good**, **inconsistent**, or **bad**, depending on whether using Lakilester's ability will always, sometimes, or never clip properly.
...
* **unpin trainer**: Multiple trainers allow you to pin information to the screen. Pressing this will unpin that information regardless of which trainer is currently selected.
