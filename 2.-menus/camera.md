...
Use `distance min` and `distance max` to control the min/max that the camera can be from Mario when not in `manual` mode. Use `move speed` and `pan speed` to adjust how fast the camera moves when adjusting it. Pressing `reset` will move the camera back to roughly where it would be with the free cam disabled.
